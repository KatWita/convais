...
app.get('/api/blogs', async c => {
	...
	try {
		...
		if (page && limit) {
			...
			results = await db.query.blogs.findMany({ where: combinedWhere, limit, offset })
		} else {
			results = allBlogs
		}
		...
		return c.json({ data: blogsWithComments, totalItems })
	} catch (e) {
...
app.post('/api/comments', async c => {
	try {
		...
		const newComment = await db
			.insert(comments)
			.values({ blogId, author, comment: finalComment, avatar, parentId: finalParentId, createdAt: new Date() })
			.returning()
